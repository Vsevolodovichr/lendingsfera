  ...
  return (
    ...
    >
          ...
          {SHOTS.map((s, i) => {
            ...
            return (
              <button
                key={s.k}
                onClick={() => setActive(i)}
                className="h-9 px-3.5 rounded-full text-[12px] font-medium transition-all"
                style={{
      ...
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <Reveal>
          <div className="surface-pop rounded-3xl p-4 md:p-5 relative overflow-hidden">
            <div className="flex items-center gap-2 mb-3 text-[11px] text-[var(--muted-foreground)]">
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "var(--surface)", border: "1px solid var(--border-soft)" }}>
      <div className="flex items-center gap-1.5 px-3 h-8" style={{ background: "var(--surface-3)" }}>
        ...
        <div className="ml-3 h-5 flex-1 max-w-xs rounded surface-card text-[9px] flex items-center px-2 text-[var(--muted-foreground)]">
          app.hatosfera.ua
        </div>
      </div>
      <div className="grid grid-cols-[140px_1fr] min-h-[320px]">
        ...
        <main className="p-4 space-y-3">
          <div className="grid grid-cols-4 gap-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="rounded-lg p-2.5" style={{ background: "var(--surface-2)", border: "1px solid var(--border-soft)" }}>
                <div className="text-[9px] text-[var(--muted-foreground)] mb-1">KPI {i + 1}</div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {["Нові", "Перегляд", "Перемовини", "Угода"].map((s, i) => (
              <div key={s} className="rounded-lg p-2.5" style={{ background: "var(--surface-2)", border: "1px solid var(--border-soft)" }}>
                <div className="text-[9px] text-[var(--muted-foreground)] mb-1">{s}</div>
          ...
          <div className="rounded-lg p-3" style={{ background: "var(--surface-2)", border: "1px solid var(--border-soft)" }}>
            <div className="text-[10px] text-[var(--muted-foreground)] mb-2">Динаміка по тижнях</div>
...
function PhoneMock({ variant }: { variant: number }) {
  return (
    <div className="mx-auto rounded-[36px] p-2.5 w-[220px] h-[440px]"
      ...
      <div className="h-full w-full rounded-[28px] overflow-hidden" style={{ background: "var(--surface)" }}>
        <div className="h-6 flex items-center justify-center text-[8px] text-[var(--muted-foreground)]" style={{ background: "var(--surface-2)" }}>
          09:41
        </div>
        <div className="p-3 space-y-2">
          <div className="text-[10px] text-[var(--muted-foreground)]">Хатосфера CRM</div>
          ...
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="rounded-lg p-2 transition-all duration-500"
              ...
              <div className="text-[9px] text-[var(--muted-foreground)] mt-1">2-кімн., Київ · {(45 + i * 8)} м²</div>
