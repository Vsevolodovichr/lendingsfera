  ...
  return (
    <header className="site-header">
      <a href="#" className="flex items-center gap-2.5">
        <img src={logo} alt="Хатосфера" className="h-11 w-11 object-contain" />
        <span className="font-semibold tracking-tight text-[15px] md:text-base text-foreground">
      ...
      {open && (
        <div className="absolute top-20 right-5 left-5 z-50 surface-pop rounded-2xl p-4 lg:hidden">
          <div className="flex flex-col gap-3">
            {NAV.map((n) => (
              <a key={n.label} href={n.href} onClick={() => setOpen(false)} className="text-sm text-muted-foreground py-1">{n.label}</a>
            ))}
            <a
              href="#contact"
